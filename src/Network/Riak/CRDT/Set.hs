...
instance CRDTOp (Op Set) where
  type UpdateOp (Op Set) = SetOp.SetOp

  updateOp :: Op Set -> UpdateOp (Op Set)
  updateOp (SetMod xs ys) = SetOp.SetOp (toSeq xs) (toSeq ys)
    where
      toSeq :: Set.Set a -> Seq a
      toSeq = Seq.fromList . Set.toList

  unionOp :: Op Set -> DtOp.DtOp
...
fetch conn typ bucket key =
  ...
  where
    ...
    go (s, Just c) = (s, c)
    -- include_context was not included in request, so it should default to true
    -- Therefore, we should always get a context back
...
-- not contain a 'Set'.
fetchWith
  :: Connection -> DtFetchRequest.DtFetchRequest
  -> IO (Maybe (Set, Maybe Context))
fetchWith conn req =
